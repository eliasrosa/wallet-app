...
async function main(): Promise<void> {
	...
	const price = priceToNumber(
		documentSelector('#cards-ticker > div._card.cotacao > div._card-body > div > span').text().trim(),
	)
	const dy12m = dyToNumber(
		documentSelector('#cards-ticker > div:nth-child(2) > div._card-body > div > span').text().trim(),
	)
	const pvp = pvpToNumber(documentSelector('#cards-ticker > div._card.vp > div._card-body > span').text().trim())
	const var12m = varToNumber(
		documentSelector('#cards-ticker > div:nth-child(5) > div._card-body > div > span').text().trim(),
	)
